...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        this.eye_direction = 0; // 蛇眼睛的方向，0,1,2,3分别表示上右下左
        if (this.id === 1) this.eye_direction = 2; // 让蛇2的眼睛初始方向向下

        this.eye_dx = [ // 蛇眼睛在四个方向上的x的偏移量
            [-1, 1],
            [1, 1],
            [1, -1],
            [-1, -1],
        ];
        this.eye_dy = [ // 蛇眼睛在四个方向上的y的偏移量
            [-1, -1],
            [-1, 1],
            [1, 1],
            [1, -1],
    ...
    check_tail_increasing() { // 检查蛇尾是否会增加
        if (this.step <= 10) return true;
        if (this.step % 3 === 1) return true;
        return false;
    }

    next_step() { // 将蛇的状态变为move，向direction的方向走一格
    ...
    update_move() {  
        ...
        if (distance < this.eps) { // 走到了目标点
            ...
            if (!this.check_tail_increasing()) { // 如果蛇尾不增加
                this.cells.pop(); // 去掉蛇尾
            }
        } else {
            const move_distance = this.speed * this.timedelta / 1000; // 本帧移动的距离
